...
We then propose and answer a complex question about COVID-19 vaccine developers using Wikidata.
...
\label{sec:intro}
The World Wide Web, originally designed for human use, has evolved
...
make use of Linked Data for answering complex, cross-domain questions~\cite{tbl2009}.
...
precisely where the Semantic Web shines: it requires joining
heterogeneous, specialised databases in a single, structured, reproducible
query, something that Large Language Models (LLMs) cannot reliably do.
...
\begin{enumerate}[label=\textbf{Q\arabic*.}]
  ...
  \item Propose and answer a new complex question using Wikidata: which organisations
...
The Semantic Web stack directly addresses these limitations (and also predates LLMs).
...
coherently. For these steps, LLMs can be used as assistants.  

% ══════════════════════════════════════════════════════════════════════════════
\newpage
\section{Question 1: Biomedical Federated Query}
\label{sec:q1}
We start with the question posed by Tim Berners-Lee, requiring connecting biological databases:  
\begin{quote}
...
~  
\text{ }\\
First, we identify the relevant datasets that contain the necessary information:
\begin{itemize}
...
\end{itemize}
We also explored other datasets but they didn't provide the necessary data or were difficult to query.



\text{ }\\
The key insight is that the two conditions in the question are held in \emph{different} databases:
\begin{enumerate}
  ...
  \item Expression in pyramidal neurons lives in Bgee (via anatomical entity
        labels).
\end{enumerate}
To query both services in a single query, we found the \texttt{SERVICE} keyword 
that can delegate sub-queries to remote endpoints and join the results locally. 
The bridge between the two databases is the UniProt identifier, which
Bgee exposes through the \texttt{lscr:xrefUniprot} predicate.


\text{ }\\
We therefore propose the following query:  
...
\text{ }\\
We faced several challenges in formulating and executing this federated query:
\begin{itemize}
...
The complex question we propose is:
\begin{quote}
...
\text{ }\\
After some exploration, we identified that the necessary information is available in
\textbf{Wikidata} (\url{https://query.wikidata.org}).
Vaccine entries are linked to their developers via the \texttt{wdt:P178} (\emph{developer}) 
property, and developers are linked to their country of headquarters via
\texttt{wdt:P17} (\emph{country}). The class \texttt{wd:Q84263196} identifies COVID-19 vaccines.


\text{ }\\
Therefore, we propose the following SPARQL query to retrieve the desired information:
...
\text{ }\\
This query returns a list of COVID-19 vaccines, their developers, and their countries as such:  
\begin{center}
\begin{tabular}{lll}
\toprule
...
\text{ }\\
Mixing LLMs and Semantic Web, ChatGPT was used to \emph{suggest} which
...
an answer (exposing the exact RDF triples that support it), SPARQL's \texttt{CONSTRUCT} form builds an explicit sub-graph. 
This sub-graph constitutes a machine-readable \emph{proof} or justification.
...
\toprule
\textbf{Linked Data} & \textbf{LLMs} \\
\midrule
Structured, machine-readable data & Natural language understanding \\
Up-to-date & Static knowledge (training cut-off) \\
...
A practical pipeline proceeds as follows:
\begin{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{No reasoning}: Linked Data cannot interpret
