...
\title{BandDPER\\\large Band-Aware Dual-Perspective Evaluation ResNet}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{components/dual_perspective_flowchart.png}
...
\begin{itemize}
  ...
  \item Unicorn escape routes (counted explicitly as a scalar signal)
...
\noindent\textbf{Note on the name}: ``Dual-Perspective'' refers to player-perspective symmetry enforced via shared Siamese weights. The architecture deliberately breaks spatial translation equivariance in two places: fixed band-map channels (4--6) encode absolute square identity, and channel 15 recentres around the unicorn.
...
The forced-pass bit bypasses the trunk entirely via a single learned scalar weight \texttt{w\_pass} added directly to the output before \texttt{tanh}. This is intentional: \texttt{tanh} bounds the sum correctly, and since \texttt{w\_pass} is initialised to zero and trained from data, it learns an appropriate magnitude without requiring additional scaling.

\newpage
\subsubsection{Shared Spatial Encoder}
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.80\textheight,keepaspectratio]{components/SSE.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.58\textheight,keepaspectratio]{components/residual_trunk.png}
...
The current PyTorch implementation applies \texttt{Dropout(0.2)} after each linear layer in every ResBlock and before the first linear layer in the output head. This regularizes the bootstrapped self-play training setup without changing the inference-time graph structure.
...
\begin{itemize}
  ...
  \item \textbf{Why dropout in ResBlocks}: The model is trained on bootstrapped self-play labels, so dropout helps reduce overfitting without removing the residual signal or changing the inference-time architecture.
...
The existing alpha-beta engine is used at depth 5--7 to label positions with ground-truth scores. The network learns to approximate these deep search evaluations. Terminal positions (unicorn captured) are labelled exactly 1.0. All other positions are normalised as \texttt{score\_label = minimax(board, depth=5) / MATE\_SCORE}, where \texttt{MATE\_SCORE} is the score returned by the engine for an immediate win (a large constant, e.g. \texttt{100\_000}). This normalises scores to $[-1, 1]$.

The data generation procedure:
\begin{enumerate}
  \item Reset the board to a random legal opening configuration.
  \item Play a mixed game: each ply either selects a random legal move (probability 0.7) or the engine's best move at depth 2 (probability 0.3), to generate diverse positions.
  \item At each ply after the 4th, label the current position with probability 0.3 using the depth-5 engine.
  \item Continue until the game ends or 100 plies are reached.
\end{enumerate}

Approximately \textbf{50K--100K labeled positions} are sufficient for the network to learn meaningful patterns without overfitting. Training examples store \texttt{(board\_state, last\_landing\_band, label)} triples, not just \texttt{(board\_state, label)}, because both the departure constraint mask and the forced-pass signal require the last landing band. Forced-pass positions are rare ($\sim$2--5\% of positions); the direct shortcut ensures they receive strong gradient despite low frequency. Iterative re-bootstrap rounds at depth 6--8 using the trained network as evaluation function can further improve label quality.
...
We use \textbf{AdamW} (\texttt{lr=1e-3}, \texttt{weight\_decay=1e-4}) with \textbf{CosineAnnealingWarmRestarts} to escape plateaus during iterative bootstrapping rounds. We also apply \textbf{gradient clipping} (\texttt{clip\_grad\_norm\_=1.0}) to prevent exploding gradients.
...
We propose adding a \textbf{policy head} that outputs a probability distribution over the $\sim$96 possible moves (due to broken-path movement reaching 4/8/16 unique destinations for band 1/2/3 respectively from interior squares). This head is used purely for move ordering in alpha-beta search.
...
\begin{enumerate}
  ...
  \item Call the policy head to obtain move probabilities; sort moves by descending probability.
  \item Run alpha-beta with well-ordered moves for stronger pruning
...
\begin{table}[htbp]
  ...
  \begin{tabular}{L{0.30\textwidth} L{0.12\textwidth} L{0.16\textwidth} L{0.30\textwidth}}
    ...
    \midrule
    Conv layers (encoder) & $\sim$33K & INT8 & Weights well-distributed; ClippedReLU bounds activations \\
    Linear proj ($1152\to128$) & $\sim$148K & INT8 & Largest layer; most to gain \\
    ResBlock linears & $\sim$402K & INT8 & Clean bounded range after ClippedReLU \\
...
// Target: ~2--5\,\textmu s inference per position on modern hardware (to be benchmarked)
\end{lstlisting}
