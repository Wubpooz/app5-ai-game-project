...
Le code source est structuré en plusieurs packages modulaires favorisant la séparation des responsabilités :
\begin{itemize}
    \item \textbf{Package \texttt{game}} : Contient les abstractions fondamentales du jeu d'Escampe. \texttt{EscampeBoard} modélise le plateau et les règles de transition d'états ; \texttt{EscampeMove} gère l'encodage des déplacements ; et \texttt{EscampeAIPlayer} implémente l'interface \texttt{IJoueur} pour orchestrer l'évaluation du meilleur coup et le temps restant.
    \item \textbf{Package \texttt{algorithms}} : Regroupe l'intelligence décisionnelle. Il est divisé en sous-packages : \texttt{algorithms.search} implémente les algorithmes d'exploration (\texttt{NegamaxKH}, \texttt{AlphaBetaKH}) régis par la classe d'ordonnancement \texttt{MoveOrderer} (Killer Moves + History Heuristic) ; \texttt{algorithms.evaluation} encapsule l'heuristique linéaire (\texttt{Heuristic}, \texttt{HeuristicConfig}) ; \texttt{Opening} gère la bibliothèque de départ ; et \texttt{TimeManager} s'occupe du budget temporel.
    \item \textbf{Package \texttt{ranking}} : Dédié à la validation et à l'optimisation. Il contient \texttt{TournamentRunner} (simulations de tournois locaux), \texttt{AIRankingSystem} (calcul du classement Elo/SPRT), et \texttt{SimplexTuner}/\texttt{ParameterTuner} (calibrations SPSA).
    \item \textbf{Package \texttt{io}} : Fournit les points d'entrée d'exécution. \texttt{ClientJeu} assure la communication réseau TCP avec l'arbitre externe ; \texttt{Solo} permet de lancer une partie locale directe pour le débogage ; et \texttt{Applet} gère le rendu graphique.
\end{itemize}
